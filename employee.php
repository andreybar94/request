<?php
		...
		$tblname	=	"employee";			//Объявление имени таблицы
		$tblindex	=	"surname"
			.	","	.	"name"
			.	","	.	"patronym"
			.	","	.	"post"
		;		//Объявление индексируемого поля
		$tblKeyField	=	"ID_employee";	// Объявление ключевого поля
	...
	if (isset($_POST['surname_add']) && isset($_POST['name_add'])){
			$_POST['surname_add']	=	trim(htmlspecialchars(stripslashes($_POST['surname_add'])));
			$_POST['name_add']		=	trim(htmlspecialchars(stripslashes($_POST['name_add'])));
			$_POST['patronym_add']	=	trim(htmlspecialchars(stripslashes($_POST['patronym_add'])));
			$_POST['post_add']	=	trim(htmlspecialchars(stripslashes($_POST['post_add'])));
			
			if (!empty($_POST['surname_add']) and !empty($_POST['name_add'])){
		...
		// Изменение записи
		if (isset($_POST['surname_upd']) && isset($_POST['name_upd'])){
			$_POST['surname_upd']	= trim(htmlspecialchars(stripslashes($_POST['surname_upd'])));
			$_POST['name_upd']		= trim(htmlspecialchars(stripslashes($_POST['name_upd'])));
			$_POST['patronym_upd']	= trim(htmlspecialchars(stripslashes($_POST['patronym_upd'])));
			$_POST['post_upd']	=	trim(htmlspecialchars(stripslashes($_POST['post_upd'])));
			if (!empty($_POST['surname_upd']) and !empty($_POST['name_upd'])){
				// Проверка на присутствие аналогичной записи в таблице
    	...
    	<main class="container">
					...
					if ($_POST['input'] == 'del'){ // Удаление
						if (!isset($_POST['chooseRow'])){
							...
							$query_record_del	=	mysqli_query($link,
								"select delEmployee(".$_POST['chooseRow'].")"
						...
						if ($_POST['input'] == 'add'){ // Добавление		
							...
							if (!isset($_POST['chooseRow'])){
								...
								drawEditorForm(
									...
									.editorField('text', array($tableColumns[2], $tableHeaders[2]), $list_of_fields, $_POST['input'], $line_content, 'required',$fields)
									.editorField('text', array($tableColumns[3], $tableHeaders[3]), $list_of_fields, $_POST['input'], $line_content, 'required',$fields)
								);
				...
				<form method="post" action="">
					<table border="0" align="center">
						...
						<?php
							drawTable($tblname,	$tblindex,	array($tableColumns, $tableHeaders),	'ID_employee != -1');
								?>
				</form>
				<br/>
				<p class="error">
