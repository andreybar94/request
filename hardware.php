<?php
		...
		$tblname	=	"hardware";			//Объявление имени таблицы
		$tblindex	=	"model"
			.	","	.	"variant";
			//Объявление индексируемого поля
		$tblKeyField	=	"ID_hardware";	// Объявление ключевого поля
		...
		if (isset($_POST['model_add']) && isset($_POST['variant_add'])){
			$_POST['model_add']	=	trim(htmlspecialchars(stripslashes($_POST['model_add'])));
			$_POST['variant_add']		=	trim(htmlspecialchars(stripslashes($_POST['variant_add'])));
						
			if (!empty($_POST['variant_add']) and !empty($_POST['model_add'])){
		...
		// Изменение записи
		if (isset($_POST['model_upd']) && isset($_POST['variant_upd'])){
			$_POST['model_upd']	= trim(htmlspecialchars(stripslashes($_POST['model_upd'])));
			$_POST['variant_upd']		= trim(htmlspecialchars(stripslashes($_POST['variant_upd'])));
			if (!empty($_POST['model_upd']) and !empty($_POST['variant_upd'])){
	...
	<body>
    	...
    	<main class="container">
					...
					if ($_POST['input'] == 'del'){ // Удаление
						if (!isset($_POST['chooseRow'])){
							...
							$query_record_del	=	mysqli_query($link,
								"select delHardware(".$_POST['chooseRow'].")"
						...
						if ($_POST['input'] == 'add'){ // Добавление		
							...
							if (!isset($_POST['chooseRow'])){
								...
								// Выполниние SQL-запроса для ввода в форму содержимого изменяемого ряда
			...
			<form method="post" action="">
				...
				<?php
					//--------------------Содержимое базы-----------------------
					drawTable($tblname,	$tblindex,	array($tableColumns, $tableHeaders),	'ID_hardware != -1');
				?>
